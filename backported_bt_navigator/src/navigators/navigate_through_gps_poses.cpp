...
#include <memory>
#include <limits>
#include "backported_bt_navigator/navigators/navigate_through_gps_poses.hpp"
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  if (!node->has_parameter("goals_blackboard_id")) {
    node->declare_parameter("goals_blackboard_id", std::string("gps_goals"));
  }

  goals_blackboard_id_ = node->get_parameter("goals_blackboard_id").as_string();

  if (!node->has_parameter("path_blackboard_id")) {
    node->declare_parameter("path_blackboard_id", std::string("gps_path"));
  }
  ...
  // Initialize odometry smoother
  odom_smoother_ = odom_smoother;

  // Initialize FromLL service client
  from_ll_to_map_client_ = std::make_unique<
      nav2_util::ServiceClientEx<robot_localization::srv::FromLL,
...
NavigateThroughGPSPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  // Convert GPS poses to map frame and update the goal poses on the blackboard
  auto map_poses = convertGPSPosesToMapPoses(goal->gps_poses);
  blackboard->set<Goals>(goals_blackboard_id_, map_poses);
}

std::vector<geometry_msgs::msg::PoseStamped>
NavigateThroughGPSPosesNavigator::convertGPSPosesToMapPoses(
  const std::vector<geographic_msgs::msg::GeoPose> & gps_poses)
{
  RCLCPP_INFO(
    logger_, "Converting %zu GPS waypoints to %s frame...",
    gps_poses.size(), global_frame_id_.c_str());

  std::vector<geometry_msgs::msg::PoseStamped> poses_in_map_frame;
  int waypoint_index = 0;

  for (const auto & curr_geopose : gps_poses) {
    ...
    if (!from_ll_to_map_client_->invoke(request, response)) {
      RCLCPP_ERROR(
        logger_,
        "fromLL service failed to convert %i-th GPS waypoint to %s frame, skipping this point!",
    ...
    poses_in_map_frame.push_back(curr_pose_map_frame);

    waypoint_index++;
  }

  RCLCPP_INFO(
    logger_, "Converted %zu GPS waypoints to %s frame",
    poses_in_map_frame.size(), global_frame_id_.c_str());
...
PLUGINLIB_EXPORT_CLASS(
  backported_bt_navigator::NavigateThroughGPSPosesNavigator,
